...
async function queryFn({page, size}: ITaskPagination): Promise<IPage<ITask[]>>{
    const response = await axiosClient.get<IPage<ITask[]>>(`/tasks/late`, {
        params: {
            "page": page,
            "size": size
        }
...
export function useListLateTasks(pagination: ITaskPagination){
    const query = useQuery({
        queryKey: ['list-late-tasks', pagination],
